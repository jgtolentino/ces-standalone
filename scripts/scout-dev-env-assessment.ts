...
class ScoutDevEnvironmentAssessor {
   ...
   */
  async runAssessment(): Promise<EnvironmentAssessment> {
   ...
   */
  private async assessDevelopmentEnvironment(): Promise<void> {
    ...
    try {
      ...
      issues.push(`Development environment check failed: ${error}`);
   ...
   */
  private async assessAINativeStack(): Promise<void> {
    ...
    if (existsSync('agents')) {
      ...
      const agentFiles = ['dash.yaml', 'keykey.yaml', 'manong.yaml', 'caca_qa_checklist.yaml'];
      let agentScore = 0;
   ...
   */
  private async assessRequiredAssets(): Promise<void> {
   ...
   */
  private async assessAgentEcosystem(): Promise<void> {
   ...
   */
  private generateReport(): EnvironmentAssessment {
   ...
   */
  private getStatus(score: number, maxScore: number): 'excellent' | 'good' | 'needs_improvement' | 'critical_issues' {
   ...
   */
  private saveReport(assessment: EnvironmentAssessment): void {
   ...
   */
  private displaySummary(assessment: EnvironmentAssessment): void {
...
export { ScoutDevEnvironmentAssessor };
